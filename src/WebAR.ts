...
export class WebAR {
  ...
  get get_color_num(): {
    ...
    cltext: string | undefined;
  } {
    return { cn: this.color_num, pth: this.col_path, cltext: this.col_text };
  }
  ...
  start(overlay_dom: string) {
    ...
    // キャンバスの背景色を設定
    descriCtx.fillStyle = "midnightblue";
    descriCtx.fillRect(0, 0, descri.width, descri.height);

    // 例: 中央にテキストを描画
    descriCtx.fillStyle = "aliceblue";
    ...
    arbutton.addEventListener("click", () => {
      ...
      const colresult = season(result.name);
      this.col_path = colresult.glb;
      this.col_text = colresult.text;
