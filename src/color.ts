...
import { Colors, glb_path, seiza } from "./Loader_Data";
...
// カラーコードから季節分類
export function season(category: string): { glb: string; text: string } {
  // let glb_Keys: string[] = [],
  ...
  if (category == "Purple") {
    ...
    text_Value = Object.values(seiza).flatMap((cate_text) =>
      Object.values(cate_text)
    );
  }
  const rndmIndex = Math.floor(Math.random() * glb_Values.length);
  // const rndmKey = glb_Keys[rndmIndex];
  const rndmValue = { glb: glb_Values[rndmIndex], text: text_Value[rndmIndex] };
  return rndmValue;
}
